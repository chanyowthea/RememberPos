...
namespace Decorate
{
    public class BlockRect : MonoBehaviour
    {
        ...
        void OnCreateItem(AssetData data, PlayerData playerData)
        {
            ...
            {
                var c = bs[i];
                var cell = Singleton._excelUtil.GetBlock(c);
                if (cell == null)
                {
                    continue;
                }
                ...
                foreach (var item in playerData._blockIdsInUse)
                {
                    if (item == cell._id)
                    {
                        vo.isEquipped = true;
                ...
                // 装备
                vo.onClick0 = () => Singleton._sceneService.EquipItem(0, vo._id, true, (int[] inUse, int[] owneds) =>
                {
                    Debug.LogError("EquipItem true");
                    UpdateEquip(inUse);
                });
                // 卸下
                vo.onClick1 = () => Singleton._sceneService.EquipItem(0, vo._id, false, (int[] inUse, int[] owneds) =>
                {
                    UpdateEquip(inUse);
                });
                _vos.Add(vo);
        ...
        void UpdateEquip(int[] ids)
        {
            ...
            {
                var use = ids[j];
                var v = _vos.Find((BlockVO temp) => temp._id == use);
                if (v != null)
                {
                    v.isEquipped = true;
                }
