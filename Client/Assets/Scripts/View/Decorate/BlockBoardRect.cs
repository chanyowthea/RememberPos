...
namespace Decorate
{
    public class BlockBoardRect : MonoBehaviour
    {
        ...
        void OnCreateItem(AssetData data, PlayerData playerData)
        {
            ...
            {
                var c = bs[i];
                var cell = Singleton._excelUtil.GetBlockBoard(c);
                if (cell == null)
                {
                    continue;
                }
                ...
                if (playerData._blockBoardIdInUse == cell._id)
                {
                    vo.count = 1;
                }

                // 装备
                vo.onClick0 = () => Singleton._sceneService.EquipItem(1, vo._id, true, (int[] inUse, int[] owneds) =>
                {
                    Debug.LogError("EquipItem true");
                    UpdateEquip(inUse);
                });
                // 卸下
                vo.onClick1 = () => Singleton._sceneService.EquipItem(1, vo._id, false, (int[] inUse, int[] owneds) =>
                {
                    UpdateEquip(inUse);
                });
                _vos.Add(vo); 
        ...
        void UpdateEquip(int[] ids)
        {
            ...
            {
                var use = ids[j];
                var v = _vos.Find((BlockBoardVO temp) => temp._id == use);
                if (v != null)
                {
                    v.count = 1;
                }
